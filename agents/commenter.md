...
- **Never comment a file a program wrote.** If the first few lines say `Code generated by`,
  `DO NOT EDIT`, `@generated` or `autogenerated`, skip it and say so. The comment would be
  discarded the next time the generator runs, and the index would then claim a description the
  file no longer carries. `chamnan-map` already excludes these from the list it asks for, so
  reaching one means it came from somewhere else — this rule is the second layer, not the first.
