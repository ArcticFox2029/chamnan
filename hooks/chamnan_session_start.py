...
REPLY_STYLES = {'concise': 'Answer without preamble, without restating the question, and without a closing offer of further help. Lead with the result, then the reasoning only where it changes what the reader would do. Keep full sentences and normal courtesy — this is about removing filler, not about sounding curt. Never abbreviate or reword code, identifiers, paths, commands, error text, commit messages or numbers: those are quoted exactly or left out. Do not re-print what a tool already returned — name what it says and move on.', 'terse': 'Lead with the result. Drop preamble, restatement and closing offers. Prefer a table or a list wherever one carries the content, and sentence fragments where a full sentence adds nothing. Never pad to seem thorough. Say uncertain things once, plainly, and move on. Never abbreviate or reword code, identifiers, paths, commands, error text, commit messages or numbers: those are quoted exactly or left out — brevity applies to the prose between them and to nothing else. Do not re-print what a tool already returned; the reader has it.'}
MAX_TOOLS = 12
