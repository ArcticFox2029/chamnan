...
| | |
|---|---|
| **Anumang modelo, anumang tagapagbigay** | Ang indeks ay payak na teksto at ipinapasa bilang konteksto. Ang modelo ang nagtatakda lamang kung gaano karami ang sulit ipadala, hindi kung saan mapupunta ang anuman. Itakda ang laki sa pamamagitan ng `--model`, `--window`, o `--profile`. Ang paglipat ng modelo ay hindi nangangailangan ng anumang muling pag-install. Kinikilala ng `--model` ang mga angkang ito sa pangalan: `claude` · `codestral` · `deepseek` · `fable` · `gemini` · `gemma` · `glm` · `gpt` · `grok` · `haiku` · `kimi` · `mistral` · `openai` · `opus` · `sonnet` — hindi pinapansin ng pagtutugma ang malaki at maliit na titik, ang mga panghati, at ang bilang ng bersyon. Sinadyang iwan ang `llama` at `qwen`: pareho silang lumalabas sa iba't ibang laki na may magkaibang kailangang badyet, kaya ang pagbanggit sa kanila ay nagbabalik ng likas na profile at isang linyang nagsasabi kung aling dalawang laki ang maaaring tinutukoy. **Gumagana pa rin ang modelong wala sa listahan**: nakukuha nito ang likas na profile at isang tala na hindi ito nakilala, at walang nabibigo. Tuwirang tinatanggap ng `--window` ang bilang at palaging tumpak. |
