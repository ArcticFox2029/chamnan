...
| | |
|---|---|
| **Model apa pun, vendor apa pun** | Indeks berupa teks biasa dan dikirim sebagai konteks. Model hanya menentukan seberapa banyak yang layak dikirim, tidak pernah menentukan apa pergi ke mana. Aturlah ukurannya dengan `--model`, `--window`, atau `--profile`. Berganti model tidak menuntut pemasangan ulang apa pun. `--model` mengenali keluarga berikut lewat namanya: `claude` · `codestral` · `deepseek` · `fable` · `gemini` · `gemma` · `glm` · `gpt` · `grok` · `haiku` · `kimi` · `mistral` · `openai` · `opus` · `sonnet` — pencocokannya mengabaikan huruf besar kecil, pemisah, dan nomor versi. `llama` dan `qwen` sengaja tidak dimasukkan: keduanya hadir dalam beberapa ukuran yang menuntut anggaran berbeda, sehingga menyebut namanya mengembalikan profil bawaan beserta satu baris yang menyebut dua ukuran yang mungkin dimaksud. **Model yang tidak ada dalam daftar tetap bekerja**: ia mendapat profil bawaan dan catatan bahwa ia tidak dikenali, dan tidak ada yang gagal. `--window` menerima angkanya langsung dan selalu tepat. |
