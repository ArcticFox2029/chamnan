...
| | |
|---|---|
| **Любая модель, любой поставщик** | Индекс — обычный текст, он передаётся как контекст. Модель меняет только то, сколько стоит отправить, но никогда — куда что попадает. Размер задаётся через `--model`, `--window` или `--profile`. Смена модели не требует переустановки. `--model` узнаёт эти семейства по имени: `claude` · `codestral` · `deepseek` · `fable` · `gemini` · `gemma` · `glm` · `gpt` · `grok` · `haiku` · `kimi` · `mistral` · `openai` · `opus` · `sonnet` — сопоставление не смотрит на регистр, разделители и номера версий. `llama` и `qwen` намеренно не включены: обе выходят в нескольких размерах, которым нужны разные бюджеты, поэтому по имени вернётся профиль по умолчанию и строка о том, какие два размера могли иметься в виду. **Модель, которой нет в списке, всё равно работает**: она получает профиль по умолчанию и пометку, что не была распознана, и ничто не падает. `--window` принимает число напрямую и всегда точен. |
