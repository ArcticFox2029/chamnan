...
| | |
|---|---|
| **任何模型，任何厂商** | 索引是纯文本，作为上下文送进去。模型只决定值得送多少，从不决定什么放在哪里。用 `--model`、`--window` 或 `--profile` 调整大小。换模型不需要重装任何东西。 `--model` 按名称识别这些系列：`claude` · `codestral` · `deepseek` · `gemini` · `gemma` · `glm` · `gpt` · `grok` · `kimi` · `mistral` · `openai` —— 匹配时忽略大小写、分隔符和版本号。`llama` 和 `qwen` 是刻意不列入的：两者都有多种规模、需要不同的预算，指定它们会得到默认档位以及一行说明它可能指哪两种规模。**不在列表里的模型照样能用**，只会拿到默认档位和一句「未识别」的说明，不会失败。`--window` 直接接受数字，永远精确。 |
