...
chamnan 只是文本加标准库 Python。索引里没有任何东西属于某一家厂商、某一个编辑器或某一种操作系统。

| | |
|---|---|
| **任何模型，任何厂商** | 索引是纯文本，作为上下文送进去。模型只决定值得送多少，从不决定什么放在哪里。用 `--model`、`--window` 或 `--profile` 调整大小。换模型不需要重装任何东西。 `--model` 按名称识别这些系列：`claude` · `codestral` · `deepseek` · `fable` · `gemini` · `gemma` · `glm` · `gpt` · `grok` · `haiku` · `kimi` · `mistral` · `openai` · `opus` · `sonnet` —— 匹配时忽略大小写、分隔符和版本号。`llama` 和 `qwen` 是刻意不列入的：两者都有多种规模、需要不同的预算，指定它们会得到默认档位以及一行说明它可能指哪两种规模。**不在列表里的模型照样能用**，只会拿到默认档位和一句「未识别」的说明，不会失败。`--window` 直接接受数字，永远精确。 |
| **macOS、Linux、Windows、WSL** | 各处都是同一个插件，只用标准库，没有东西要装。在 macOS 和 Linux 上命令直接就能跑。在 Windows 上，外壳无法执行没有扩展名的脚本，所以每个命令和钩子旁边都放着生成的 `.cmd` 垫片；垫片随插件一起发布，CI 会真正执行它们。WSL 按 Linux 处理。 |
| **多个智能体，一份索引** | Claude Code 通过会话钩子拿到内容，不会往你的项目里写文件。Gemini CLI 同样有真正的会话钩子。其他智能体会在各自读取的位置拿到一个文件，读同一位置的智能体共用同一份文件，而不是各留一份逐渐走样的副本。 |
| **Hermes Agent** | Hermes 同时也是驱动其他编码智能体的中枢，所以为它配置好的仓库往往意味着多个工具在读同一份索引。它按固定顺序寻找项目说明文件，取第一个找到的；chamnan 写的正是这个顺序里最靠前的那个文件，按 Hermes 自己公布的上限来控制大小，并且拒绝覆盖不是它写的文件。 |

## 如何安装

走哪条路，只取决于那个工具有没有会话钩子。

| | |
|---|---|
| **Claude Code** | 作为插件安装，在仓库里执行一次 bootstrap 命令。不会往你的代码里写任何东西，此后每个会话开始时索引已经在上下文里。 |
| **其他所有工具，包括 Hermes** | 先问 chamnan 检测到了什么，再告诉它要为哪个智能体写。仓库形态变了就重建索引并再写一次文件；可选的 Git 钩子会在提交时把两件事都做了。不需要 Claude Code，这些都是普通命令，插件只是其中一种投递方式，不是产品本身。如果没有指定智能体，它会打印检测结果和该用的命令，把决定权留给你，绝不凭猜测写入。 |

命令名称、智能体的完整清单以及每个智能体收到的文件，都在英文版 README 里；所有与版本相关的细节也都在那里。
