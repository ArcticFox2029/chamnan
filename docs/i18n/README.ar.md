...
chamnan نصٌّ وبايثون من المكتبة القياسية. لا شيء في الفهرس يخصّ مزوّدًا بعينه ولا محرّرًا بعينه ولا نظام تشغيل بعينه.

| | |
|---|---|
| **أي نموذج، وأي مزوّد** | الفهرس نصٌّ صِرف يُمرَّر كسياق. النموذج يغيّر فقط مقدار ما يستحقّ الإرسال، ولا يغيّر أبدًا موضع أي شيء. يُضبط الحجم بـ `--model` أو `--window` أو `--profile`. وتبديل النموذج لا يستلزم إعادة تثبيت شيء. يتعرّف `--model` على هذه العائلات بالاسم: `claude` · `codestral` · `deepseek` · `fable` · `gemini` · `gemma` · `glm` · `gpt` · `grok` · `haiku` · `kimi` · `mistral` · `openai` · `opus` · `sonnet` — والمطابقة تتجاهل حالة الأحرف والفواصل وأرقام الإصدار. أما `llama` و`qwen` فمستبعدتان عن قصد: كلتاهما تصدر بأحجام عدّة تريد ميزانيات مختلفة، فتسميتهما تُعيد الملف الافتراضي وسطرًا يبيّن أيّ حجمين قد يكون المقصود. **والنموذج غير المدرَج يعمل رغم ذلك**: يحصل على الملف الافتراضي وملاحظة بأنه لم يُعرَف، ولا يفشل شيء. و`--window` يأخذ الرقم مباشرة وهو دقيق دائمًا. |
| **macOS وLinux وWindows وWSL** | الإضافة نفسها في كل مكان، مكتبة قياسية فقط، ولا شيء يُثبَّت. على macOS وLinux تعمل الأوامر مباشرة. أما على Windows فالصدفة لا تستطيع تشغيل سكربت بلا امتداد، لذا يوضع ملف `.cmd` مولَّد بجانب كل أمر وكل خطّاف؛ تُشحن هذه الملفات مع الإضافة، وتشغّلها بيئة التكامل المستمر بنفسها. أما WSL فيتصرّف كـ Linux. |
| **وكلاء كثيرون، فهرس واحد** | يتلقّى Claude Code الكتلة عبر خطّاف جلسة، ولا يُكتب أي ملف في مشروعك. ولدى Gemini CLI خطّاف جلسة حقيقي كذلك. أما بقية الوكلاء فيتلقّى كلٌّ منهم ملفًا في المسار الذي يقرأه، والذين يقرؤون المسار نفسه يتشاركون الملف بدل أن يحتفظ كلٌّ بنسخة تنحرف عن الأخرى. |
| **Hermes Agent** | Hermes أيضًا طبقة تحكّم تقود وكلاء برمجة آخرين، لذا فإن مستودعًا مهيّأً له يعني غالبًا أدوات عدّة تقرأ الفهرس نفسه. يبحث عن تعليمات المشروع بترتيب ثابت ويأخذ أول ما يجده؛ وchamnan يكتب الملف الذي يتصدّر ذلك الترتيب، ويضبط حجمه على الحدّ الذي يوثّقه Hermes نفسه، ويرفض الكتابة فوق ملف لم يكتبه هو. |

## كيفية التثبيت

أي طريق تسلكه يتوقّف على شيء واحد فقط: أللأداة خطّاف جلسة أم لا.

| | |
|---|---|
| **Claude Code** | ثبّته كإضافة، ونفّذ أمر التهيئة مرّة واحدة داخل مستودع. لا يُكتب شيء في شفرتك، وبعدها تبدأ كل جلسة والفهرس حاضر في السياق. |
| **كل ما عدا ذلك، ومنه Hermes** | اسأل أولًا عمّا اكتشفه chamnan، ثم أخبره لأي وكيل يكتب. وحين يتغيّر شكل المستودع، أعد بناء الفهرس واكتب الملف من جديد؛ وثمّة خطّاف Git اختياري يفعل الأمرين عند الإيداع. ولا حاجة إلى Claude Code: هذه أوامر عادية، والإضافة مجرّد طريق تسليم واحد لا المنتج نفسه. وإن لم تسمِّ وكيلًا، طبع ما اكتشفه والأمر الذي يناسب، وترك القرار لك. ولا يكتب أبدًا بناءً على تخمين. |

أسماء الأوامر، وقائمة الوكلاء كاملة، والملف الذي يتلقّاه كلٌّ منهم — كلها في ملف README الإنجليزي، حيث تعيش كل تفصيلة مرتبطة بإصدار.
