...
| | |
|---|---|
| **N'importe quel modèle, n'importe quel fournisseur** | L'index est du texte brut, envoyé comme contexte. Le modèle ne change que la quantité qu'il vaut la peine d'envoyer, jamais l'endroit où va quoi que ce soit. On règle la taille avec `--model`, `--window` ou `--profile`. Changer de modèle n'oblige à réinstaller quoi que ce soit. `--model` reconnaît ces familles par leur nom : `claude` · `codestral` · `deepseek` · `gemini` · `gemma` · `glm` · `gpt` · `grok` · `kimi` · `mistral` · `openai` — la comparaison ignore la casse, les séparateurs et les numéros de version. `llama` et `qwen` en sont volontairement absents : tous deux existent en plusieurs tailles qui réclament des budgets différents, donc les nommer renvoie le profil par défaut et une ligne indiquant les deux tailles possibles. **Un modèle absent de la liste fonctionne quand même** : il obtient le profil par défaut et une note disant qu'il n'a pas été reconnu, et rien n'échoue. `--window` prend le nombre directement et reste toujours exact. |
