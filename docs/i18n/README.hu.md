...
| | |
|---|---|
| **Bármelyik modell, bármelyik szállító** | Az index egyszerű szöveg, és kontextusként megy át. A modell csak azt változtatja, mennyit érdemes elküldeni, azt soha, hogy mi hová kerül. A méretet a `--model`, `--window` vagy `--profile` állítja. Modellt váltani nem jár újratelepítéssel. A `--model` ezeket a családokat ismeri fel név szerint: `claude` · `codestral` · `deepseek` · `fable` · `gemini` · `gemma` · `glm` · `gpt` · `grok` · `haiku` · `kimi` · `mistral` · `openai` · `opus` · `sonnet` — az egyeztetés nem törődik a kis- és nagybetűkkel, az elválasztókkal és a verziószámokkal. A `llama` és a `qwen` szándékosan kimaradt: mindkettő több méretben jelenik meg, amelyek eltérő keretet kívánnak, így a nevük az alapértelmezett profilt adja vissza és egy sort arról, melyik két méretre gondolhattál. **A listán nem szereplő modell is működik**: megkapja az alapértelmezett profilt és egy megjegyzést, hogy nem ismerte fel, és semmi sem hiúsul meg. A `--window` közvetlenül veszi a számot, és mindig pontos. |
