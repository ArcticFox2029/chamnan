...
| | |
|---|---|
| **Dowolny model, dowolny dostawca** | Indeks to zwykły tekst przekazywany jako kontekst. Model zmienia tylko to, ile warto wysłać, nigdy zaś to, gdzie co trafia. Rozmiar ustawia się przez `--model`, `--window` lub `--profile`. Zmiana modelu nie wymaga ponownej instalacji. `--model` rozpoznaje te rodziny po nazwie: `claude` · `codestral` · `deepseek` · `fable` · `gemini` · `gemma` · `glm` · `gpt` · `grok` · `haiku` · `kimi` · `mistral` · `openai` · `opus` · `sonnet` — dopasowanie pomija wielkość liter, separatory i numery wersji. `llama` i `qwen` są celowo pominięte: obie wychodzą w kilku rozmiarach wymagających różnych budżetów, więc podanie nazwy zwraca profil domyślny i wiersz mówiący, o które dwa rozmiary mogło chodzić. **Model spoza listy i tak działa**: dostaje profil domyślny oraz notkę, że nie został rozpoznany, i nic nie zawodzi. `--window` przyjmuje liczbę wprost i jest zawsze dokładny. |
