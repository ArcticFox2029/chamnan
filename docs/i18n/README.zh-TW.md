...
chamnan 只是文字加標準函式庫的 Python。索引裡沒有任何東西屬於某一家廠商、某一個編輯器或某一種作業系統。

| | |
|---|---|
| **任何模型，任何廠商** | 索引是純文字，當作脈絡送進去。模型只決定值得送多少，從不決定什麼放在哪裡。用 `--model`、`--window` 或 `--profile` 調整大小。換模型不需要重裝任何東西。 `--model` 按名稱辨識這些系列：`claude` · `codestral` · `deepseek` · `fable` · `gemini` · `gemma` · `glm` · `gpt` · `grok` · `haiku` · `kimi` · `mistral` · `openai` · `opus` · `sonnet` —— 比對時忽略大小寫、分隔符號與版本號。`llama` 與 `qwen` 是刻意不列入的：兩者都有多種規模、需要不同的預算，指定它們會得到預設檔位以及一行說明它可能指哪兩種規模。**不在清單裡的模型照樣能用**，只會拿到預設檔位和一句「未辨識」的說明，不會失敗。`--window` 直接接受數字，永遠精確。 |
| **macOS、Linux、Windows、WSL** | 各處都是同一個外掛，只用標準函式庫，沒有東西要裝。在 macOS 與 Linux 上指令直接就能跑。在 Windows 上，殼層無法執行沒有副檔名的指令稿，所以每個指令與掛鉤旁邊都放著產生的 `.cmd` 墊片；墊片隨外掛一起發布，CI 會真正執行它們。WSL 按 Linux 處理。 |
| **多個代理，一份索引** | Claude Code 透過工作階段掛鉤取得內容，不會往你的專案寫檔案。Gemini CLI 同樣有真正的工作階段掛鉤。其他代理會在各自讀取的位置拿到一個檔案，讀同一位置的代理共用同一份檔案，而不是各留一份逐漸走樣的副本。 |
| **Hermes Agent** | Hermes 同時也是驅動其他編碼代理的中樞，所以為它設定好的儲存庫往往意味著多個工具在讀同一份索引。它按固定順序尋找專案說明檔案，取第一個找到的；chamnan 寫的正是這個順序裡最前面的那個檔案，按 Hermes 自己公布的上限控制大小，並且拒絕覆蓋不是它寫的檔案。 |

## 如何安裝

走哪條路，只取決於那個工具有沒有工作階段掛鉤。

| | |
|---|---|
| **Claude Code** | 以外掛安裝，在儲存庫裡執行一次 bootstrap 指令。不會往你的程式碼寫任何東西，此後每個工作階段開始時索引已經在脈絡裡。 |
| **其他所有工具，包含 Hermes** | 先問 chamnan 偵測到了什麼，再告訴它要為哪個代理寫。儲存庫形態變了就重建索引並再寫一次檔案；可選的 Git 掛鉤會在提交時把兩件事都做了。不需要 Claude Code，這些都是普通指令，外掛只是其中一種投遞方式，不是產品本身。若沒有指定代理，它會印出偵測結果與該用的指令，把決定權留給你，絕不憑猜測寫入。 |

指令名稱、代理的完整清單以及每個代理收到的檔案，都在英文版 README 裡；所有與版本相關的細節也都在那裡。
